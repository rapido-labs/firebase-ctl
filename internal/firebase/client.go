...
func GetClientStore(ctx context.Context) (*ClientStore, error) {
	...
	if err != nil {
		return &ClientStore{remoteConfigClient: nil, customFs: &customFs{afero.NewOsFs()}} , errors.New(fmt.Sprintf("error creating firebase remote config app: %v", err.Error()))
	}
	client, err := firebaseApp.RemoteConfig(ctx)
	if err != nil {
		return &ClientStore{remoteConfigClient: client, customFs: &customFs{afero.NewOsFs()}},  errors.New(fmt.Sprintf("error creating firebase remote config client: %v", err.Error()))
	}
